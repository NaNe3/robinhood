...
function SignUp({ setSignIn, setAuth }) {
    ...
    const shoot = () => {
        ...
        fname == '' ? document.getElementById('fname').style.border = "1px solid red" : document.getElementById('fname').style.border = "1px solid lightgray"
        lname == '' ? document.getElementById('lname').style.border = "1px solid red" : document.getElementById('lname').style.border = "1px solid lightgray"
    }

    const handleSubmit = (e) => {
        if (email != '' && pass != '' && fname != '' && lname != '') {
            fetch(`http://localhost:3000/signup?name=` + fname + " " + lname + `&email=` + email + "&password=" + pass, {
                method: 'POST',
                cache: 'no-cache',
                cors: 'no-cors'
            })
                ...
                .then(data => {
                    if (data._id) {
                        ...
                        sessionStorage.setItem('crypto', JSON.stringify([]))
                        sessionStorage.setItem('lists', JSON.stringify([]))
                        sessionStorage.setItem('bp', 0)
                        setAuth(true)
                    } else {
                        alert("Error!")
                    }
